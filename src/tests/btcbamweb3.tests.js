...
const Sweb3 = require('../btcbamweb3');
...
const { getBtcbamRPCAddress, getDefaultBtcbamAddress, getWalletPassphrase, isWalletEncrypted } = require('../../test/utils');

const { assert } = chai;

console.log(`Your Btcbam RPC address is ${getBtcbamRPCAddress()}`);
console.log(`Your Default Btcbam address is ${getDefaultBtcbamAddress()}`);

describe('Sweb3', () => {
  const Btcbam_ADDRESS = getDefaultBtcbamAddress();
  let sweb3;

  beforeEach(() => {
    sweb3 = new Sweb3(getBtcbamRPCAddress());
  });

  /** ******** MISC ********* */
  describe('isConnected()', () => {
    it('returns true when connected', async () => {
      assert.isTrue(await sweb3.isConnected());
    });
  });
  ...
  describe('getBlockchainInfo()', () => {
    it('returns the block info', async () => {
      const res = await sweb3.getBlockchainInfo();
      assert.isDefined(res);
      assert.isObject(res);
  ...
  describe('getBlockCount()', () => {
    it('returns the blockcount', async () => {
      const res = await sweb3.getBlockCount();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('getBlockHash()', () => {
    it('returns the block hash', async () => {
      const res = await sweb3.getBlockHash(0);
      assert.isDefined(res);
      assert.isString(res);
    });
  });
  ...
  describe('listContracts()', () => {
    it('returns the array of deployed contracts', async () => {
      const res = await sweb3.listContracts();
      assert.isDefined(res);
      assert.isObject(res);
    });
  });
  ...
  describe('searchLogs()', () => {
    ...
    it('throws if fromBlock is not a number', async () => {
      assert.throws(() => sweb3.searchLogs(
        'a', 50100, [],
  ...
  describe('getPeerInfo()', () => {
    it('returns the Node info', async () => {
      const res = await sweb3.getPeerInfo();
      assert.isDefined(res);
      assert.isArray(res);
  ...
  describe('getHexAddress()', () => {
    it('returns the hex address', async () => {
      const hexDecodedAddress = bs58.decode(Btcbam_ADDRESS).toString('hex');
      const hexadecimalAddress = await sweb3.getHexAddress(Btcbam_ADDRESS);
  ...
  describe('fromHexAddress()', () => {
    it('returns the Btcbam address', async () => {
      const BtcbamAddress = await sweb3.fromHexAddress('17e7888aa7412a735f336d2f6d784caefabb6fa3');
      assert.isString(BtcbamAddress);
      assert.lengthOf(BtcbamAddress, 34);
    });
  });
  ...
  describe('validateAddress()', () => {
    it('returns an object validating the address', async () => {
      let res = await sweb3.validateAddress(Btcbam_ADDRESS);
      assert.isDefined(res);
      assert.isDefined(res.isvalid);

      res = await sweb3.validateAddress('helloworld');
      assert.isDefined(res);
      assert.isDefined(res.isvalid);
    });
  });
  ...
  describe('dumpPrivateKey()', () => {
    it('returns the private key', async () => {
      const address = await sweb3.getAccountAddress('');
      if (await isWalletEncrypted(sweb3)) {
        await sweb3.walletPassphrase(getWalletPassphrase(), 3600);
        assert.isTrue((await sweb3.getWalletInfo()).unlocked_until > 0);

        const res = await sweb3.dumpPrivateKey(address);
        assert.isDefined(res);
        assert.isString(res);

        await sweb3.walletLock();
        assert.isTrue((await sweb3.getWalletInfo()).unlocked_until === 0);
      } else {
        const res = await sweb3.dumpPrivateKey(address);
        assert.isDefined(res);
  ...
  describe('getAccountAddress()', () => {
    it('returns the account address', async () => {
      const res = await sweb3.getAccountAddress('');
      assert.isDefined(res);
      assert.isString(res);
      assert.isTrue(res.startsWith('b') || res.startsWith('B'));
    });
  });

  describe('getAddressesByAccount()', () => {
    it('returns the account address array', async () => {
      const res = await sweb3.getAddressesByAccount('');
      assert.isDefined(res);
      assert.isArray(res);
      assert.isTrue(_.every(res, item => item.startsWith('b') || item.startsWith('B')));
    });
  });
    ...
    it('returns the transaction info', () => {
      const res = {
        ...
        details: [
          ...
          {
            account: '',
            address: 'bKjn4fStBaAtwGiwueJf9qFxgpbAvf1xAy',
            category: 'send',
            amount: -0.23241276,
            label: 'main',
            vout: 1,
  ...
  describe('getUnconfirmedBalance()', () => {
    it('returns the unconfirmed balance', async () => {
      const res = await sweb3.getUnconfirmedBalance();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('listAddressGroupings()', () => {
    it('returns an array of address groupings', async () => {
      const res = await sweb3.listAddressGroupings();
      assert.isDefined(res);
      assert.isArray(res);
      ...
      if (!_.isEmpty(res)) {
        ...
        if (!_.isEmpty(innerArr)) {
          ...
          qAssert.isBtcbamAddress(item[0]);
  ...
  describe('listLockUnspent()', () => {
    it('returns an array of unspendable outputs', async () => {
      const res = await sweb3.listLockUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('listUnspent()', () => {
    it('returns an unspent output array', async () => {
      const res = await sweb3.listUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('walletLock()', () => {
    it('locks the encrypted wallet', async () => {
      if (await isWalletEncrypted(sweb3)) {
        await sweb3.walletPassphrase(getWalletPassphrase(), 3600, true);
        assert.isTrue((await sweb3.getWalletInfo()).unlocked_until > 0);
  ...
  !_.includes(process.argv, '--cleanenv') ? describe.skip : describe('cleanEnv tests', () => {
    describe('getNewAddress()', () => {
      it('returns a new Btcbam address', async () => {
        const res = await sweb3.getNewAddress('');
        assert.isDefined(res);
        assert.isString(res);
        assert.isTrue(res.startsWith('b') || res.startsWith('B'));
      });
    ...
    describe('backupWallet()', () => {
      it('backup the wallet', async () => {
        const res = await sweb3.backupWallet(path.join(__dirname, '../../test/data/backup.dat'));
    ...
    describe('importWallet()', () => {
      it('throw an error if importing a non-existent file', async () => {
        try {
          await sweb3.importWallet(path.join(__dirname, '../../test/data/backup.dat'));
        } catch (err) {
      ...
      it('import the wallet from a wallet dump file', async () => {
        const res = await sweb3.importWallet(path.join(__dirname, '../../test/data/backup.dat'));
        assert.notTypeOf(res, 'Error');
      });
    });
  });
